...
<?php
	/*
		Script to find the file path to the current user picture.
		If no profile picture is found, a default picture is used.
	*/
	function profilePictureFileName() {
		// Regular expression for expected file path for current user's profile picture.
		$filenameRexEx = "user-pictures/profile-picture" . $_SESSION[ 'user_id' ] . ".*";
		
		$filenameArray = glob( $filenameRexEx );
		
		if( !empty( $filenameArray )){
			// There should be at most one profile picture for a given user, so return the first one found.
			return $filenameArray[0];
		}
		
		// If no profile picture is found, return path to default picture.
		return "images/logo_globuzzer.png";
	}
?>
...
<script type="text/babel">
	...
	function NavMenu() {
        ...
        <div className="Pic_Name_Location">
          <img className="profile_pic" src="<?php echo profilePictureFileName(); ?>" alt="profile picture" width="100" height="100"/>
          <div className="profile_name"><?php echo $_SESSION[ 'fname' ] . " " . $_SESSION[ 'lname' ] ?></div>
          <div className="profile_location"><?php echo $_SESSION[ 'location' ] . ", " . $_SESSION[ 'country' ] ?></div>
        </div>
        <div className="white_line"></div>
		...
		return (
			<div className="optional">
         	...
         	<label htmlFor="introduce">Introduction</label>
					<textarea className="introduce_text" rows="6" cols="50" name="introduction" placeholder="Tell us about more yourself.."><?php echo $_SESSION[ 'introduction' ]; ?></textarea>
				</div>
			</div>
	)}
